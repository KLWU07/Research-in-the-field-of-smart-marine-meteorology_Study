...
train_end = '2024-05-01 18:00:00'
test_start = '2024-05-02 00:00:00'
...
# 8. 训练Lasso回归模型
# alpha为正则化参数，值越大正则化强度越高，可能导致部分系数变为0
model = Lasso(alpha=0.1, max_iter=10000)  # 增加迭代次数确保收敛，可调整alpha值
model.fit(X_train_scaled, y_train)

# 输出模型系数（Lasso可能会将部分系数压缩至0）
print(f"Lasso回归系数: {model.coef_}")
print(f"Lasso回归截距: {model.intercept_}")
